...
def _matches_parameter(name: str, *parameters: str) -> bool:
    return any(name == value or name.endswith("_" + value) for value in parameters)


def _file_argument_help(name: str) -> str | None:
...
def _rule_argument_help(name: str, subject: str) -> str | None:
...
def _fallback_suffix_argument_help(name: str, subject: str) -> str | None:
...
def _automatic_argument_help(option: str, kwargs: Mapping[str, Any]) -> str:
    ...
    if name in {"model", "model_a", "model_b", "resume_state"} or name.endswith(
        "_model"
    ):
...
def finite_float(value: Any) -> float:
    ...
    try:
        ...
        raise argparse.ArgumentTypeError("expected a floating-point number") from exc
...
class CdskitArgumentParser(argparse.ArgumentParser):
    ...
    def add_deprecated_alias(self, old_option: str, new_option: str) -> None:
    ...
    def add_argument(self, *args: Any, **kwargs: Any) -> argparse.Action:
        ...
        if "help" not in kwargs and any(
            isinstance(value, str) and value.startswith("-") for value in args
        ):
            long_options = [
                value
                for value in args
    ...
    def _normalize_deprecated_aliases(
        self,
        args: Iterable[str] | None,
    ) -> list[str]:
        normalized = list(sys.argv[1:]) if args is None else list(args)
        warned: set[str] = set()
