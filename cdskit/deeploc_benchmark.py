...
DEEPLOC21_URLS = {
    ...
    "hpa_test": (
        "https://services.healthtech.dtu.dk/services/DeepLoc-2.1/data/hpa_testset.csv"
    ),
    "sorting_signals": (
        "https://services.healthtech.dtu.dk/services/DeepLoc-2.1/data/"
...
def download_deeploc21_data(out_dir, names=None, timeout_sec=120):
    ...
    for name in names:
        ...
        with atomic_output_path(out_path) as temporary:
            ...
            with (
                urllib_request.urlopen(request, timeout=int(timeout_sec)) as resp,
                open(temporary, "wb") as out_file,
            ):
                ...
                if (
                    ...
                    raise ValueError(
                        "DeepLoc download exceeds the 256 MiB safety limit."
                    )
                while True:
...
def prepare_deeploc21_sorting_signal_tsv(csv_path, out_tsv_path):
    ...
    for row in rows:
        raw_types = [
            value.strip()
            for value in str(row.get("Types", "")).split("_")
            if value.strip() != ""
        ]
        labels = [
            value for value in raw_types if value in DEEPLOC_SORTING_SIGNAL_LABELS
        ]
        out_row = {
...
def prepare_all_deeploc21(data_dir, out_dir):
    ...
    report["localization_train_validation"] = prepare_deeploc21_localization_tsv(
        ...
        out_tsv_path=os.path.join(
            out_dir, "deeploc21_localization_train_validation.tsv"
        ),
...
def _active_labels_from_row(row, labels, label_col):
    ...
    return [label for label in labels if _to_bool01(row.get(label, "0"))]
...
def _rare_threshold_objective_by_class(label_matrix, labels, dl_params=None):
    ...
    if objective not in ["mcc", "f0.5", "f1", "f2"]:
        raise ValueError(
            "Unsupported --rare_label_threshold_objective: {}".format(objective)
        )
...
def compute_multilabel_metrics(y_true, y_pred, labels):
    ...
    if len(observed_f1_values) == 0:
        observed_macro_f1 = float(
            np.mean(np.asarray(macro_f1_values, dtype=np.float64))
        )
    else:
        observed_macro_f1 = float(
            np.mean(np.asarray(observed_f1_values, dtype=np.float64))
        )
...
def _predict_model_on_rows(model, rows):
    ...
    if model_type == "multilabel_cnn_v1":
        ...
        batch_size = int(
            model.get("metadata", {}).get("cnn_params", {}).get("batch_size", 512)
        )
...
def evaluate_deeploc21_train_test(
    ...
    metrics["model"] = _model_type_from_arch(model_arch=model_arch)
    metrics["n_train_rows"] = len(train_rows)
...
def _task_config(task_name, prepared_dir):
    ...
    if task == "localization":
        return {
            ...
            "train_tsv": os.path.join(
                prepared_dir, "deeploc21_localization_train_validation.tsv"
            ),
    ...
    if task == "membrane":
        return {
            ...
            "train_tsv": os.path.join(
                prepared_dir, "deeploc21_membrane_train_validation.tsv"
            ),
            "test_tsv": "",
...
def render_deeploc_benchmark_markdown(result):
    ...
    lines.append("")
    lines.append(
        "| Split | Rows | Subset acc. | Jaccard | Micro F1 | Macro F1 | Macro F1 observed | Pred/true |"
    )
    lines.append("| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |")
    for key, name in [
        ("cross_validation", "SwissProt CV"),
...
def build_parser():
    ...
    parser.add_argument(
        ...
        choices=["localization", "membrane", "sorting_signals"],
        type=str,
    )
    ...
    parser.add_argument(
        "--prepare_report_json",
        dest="report_json",
        default="data/localize_bench/deeploc21/prepare_report.json",
        type=str,
    )
    parser.add_argument(
        "--comparison_json",
        default="data/localize_bench/deeploc21/comparison.json",
        type=str,
    )
    parser.add_argument(
        "--comparison_md",
        default="data/localize_bench/deeploc21/comparison.md",
        type=str,
    )
    ...
    parser.add_argument(
        ...
        default="f1",
        choices=["f0.5", "f1", "f2", "mcc"],
        type=str,
    )
    parser.add_argument(
        "--rare_label_threshold_objective",
        default="none",
        choices=["none", "f0.5", "f1", "f2", "mcc"],
        type=str,
    )
