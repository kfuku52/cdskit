...
DEFAULT_TRAIN_TSV = (
    "data/localize_bench/deeploc21/deeploc21_localization_train_validation.tsv"
)
    ...
    for old_name, new_name, selected in [
        ...
        if selected == old_name:
            if old_name not in fieldnames:
                raise ValueError(
                    "Missing required columns in {}: {}".format(path, new_name)
                )
            warnings.warn(
                "TSV column {} is deprecated; use {} instead.".format(
                    old_name, new_name
                ),
                FutureWarning,
                stacklevel=2,
        ...
        rows.append(
            {
                ...
                "localization_labels": str(
                    row.get("localization_labels", "") or ""
                ).strip(),
...
def _feature_names_for_profile(feature_profile):
    ...
    if profile == TARGETP_FEATURE_ENSEMBLE_PROFILE["name"]:
        return [
            "targetp_feature_{}".format(i) for i in range(_targetp_feature_dim_probe())
        ]
...
def _targetp_feature_dim_probe():
    return int(
        extract_targetp_feature_ensemble_features("MAAAAAAAAAASKL", "Metazoa").shape[0]
    )
...
def predict_perox_yes_probabilities(rows, perox_model):
    ...
    if mode != "sklearn_binary":
        raise ValueError(
            "Unsupported perox_model mode for batch prediction: {}".format(mode)
        )
...
def add_homology_subset_metrics(
    metrics, prefix, rows, probabilities, hit_indices, threshold
):
...
def split_train_valid_hash_stratified(rows, validation_fraction=0.2):
    ...
    for label_groups in by_label.values():
        ...
        if len(ordered) <= 1:
            ...
            continue
        target_valid_rows = round(float(total_rows) * fraction)
...
def split_train_valid(rows, validation_partition="4", validation_fraction=0.2):
    ...
    if validation_partition.lower() in [
        "hash_stratified",
        "auto_hash",
        "stratified_hash",
    ]:
...
def leakage_report(train_rows, eval_rows):
    train_acc = {
        row["accession"] for row in train_rows if row.get("accession", "") != ""
    }
    ...
    return {
        ...
        "accession_overlap_count": len(train_acc & eval_acc),
        "exact_sequence_overlap_count": len(train_seq & eval_seq),
    }
...
def sequence_label_conflict_report(rows):
    ...
    conflicts = [
        dict(
            ...
            accession_count=len(entry["accessions"]),
    ...
    conflicts.sort(
        key=lambda row: (row["row_count"], row["sequence_digest"]), reverse=True
    )
    duplicate_groups = [entry for entry in by_seq.values() if entry["row_count"] > 1]
    return {
        "rows": len(rows),
        "unique_sequences": len(by_seq),
        "duplicate_sequence_count": len(duplicate_groups),
        "duplicate_sequence_rows": int(
            sum(entry["row_count"] for entry in duplicate_groups)
        ),
        "duplicate_sequence_excess_rows": int(
            sum(entry["row_count"] - 1 for entry in duplicate_groups)
        ),
        "conflicting_sequence_count": len(conflicts),
...
def _write_rows_fasta_with_ids(rows, path, ids):
    with open(path, "w", encoding="utf-8") as out:
        for row, seq_id in zip(rows, ids, strict=False):
            seq = str(row.get("sequence", "") or "").strip()
...
def mmseqs_homology_report(
    ...
    with tempfile.TemporaryDirectory(
        prefix="cdskit_perox_mmseqs_", **tmp_kwargs
    ) as tmp_dir:
    ...
    hit_eval_indices = sorted(
        idx
        for idx in (_query_index_from_mmseqs_id(query_id) for query_id in hit_query_ids)
        if idx is not None
    )
    positive_query_ids = {
    ...
    report = {
        ...
        "hit_query_count": len(hit_query_ids),
        "hit_query_fraction": 0.0
        if len(eval_rows) == 0
        else float(len(hit_query_ids)) / float(len(eval_rows)),
        "positive_hit_query_count": len(hit_query_ids & positive_query_ids),
        "positive_hit_query_fraction": 0.0
        if n_pos == 0
        else float(len(hit_query_ids & positive_query_ids)) / float(n_pos),
        "negative_hit_query_count": len(hit_query_ids & negative_query_ids),
        "negative_hit_query_fraction": 0.0
        if n_neg == 0
        else float(len(hit_query_ids & negative_query_ids)) / float(n_neg),
        "best_hit_pident_max": None if len(pidents) == 0 else float(np.max(pidents)),
        "best_hit_pident_median": None
        if len(pidents) == 0
        else float(np.median(pidents)),
        "top_hits": top_hits,
    }
...
def mmseqs_cluster_assignments(
    ...
    seq_ids = [
        "s{}_{}".format(i, _sanitize_fasta_id(row.get("accession", "")))
        for i, row in enumerate(rows)
    ]
    ...
    if str(tmp_root or "").strip():
        ...
        tmp_kwargs["dir"] = str(tmp_root)
    with tempfile.TemporaryDirectory(
        prefix="cdskit_perox_cluster_", **tmp_kwargs
    ) as tmp_dir:
        tmp_path = Path(tmp_dir)
...
def assign_cluster_folds(rows, cluster_ids, n_folds=5):
    ...
    cluster_stats = {}
    for i, (row, cluster_id) in enumerate(zip(rows, cluster_ids, strict=False)):
    ...
    fold_stats = [{"positive": 0, "negative": 0, "total": 0} for _ in range(n_folds)]
    fold_ids = [0 for _ in rows]
    for _cluster_id, stats in ordered:
    ...
    return fold_ids, {
        ...
        "folds": [
            dict({"fold_id": int(fold)}, **fold_stats[fold]) for fold in range(n_folds)
        ],
    }


def _rows_by_mask(rows, mask):
    return [row for row, keep in zip(rows, mask.tolist(), strict=False) if bool(keep)]
...
def run_cluster_oof_perox_benchmark(
    ...
    if method == "singleton":
        ...
        cluster_report = {
            ...
            "singleton_cluster_count": len(rows),
        }
    elif method == "mmseqs":
...
def _serializable_perox_model_summary(perox_model):
    return {
        key: value for key, value in perox_model.items() if key not in ["classifier"]
    }
...
def write_predictions_tsv(path, rows, probabilities, threshold):
    ...
    output_rows = []
    for row, prob in zip(rows, probabilities.tolist(), strict=False):
        signals = detect_perox_signals(row.get("sequence", ""))
        output_rows.append(
            {
...
def run_deeploc21_perox_benchmark(
    ...
    if _str_to_bool(exclude_external_from_train) and external_rows:
        ...
        train_exclusion_report = {
            "enabled": False,
            "input_rows": len(original_all_rows),
            "eval_rows": len(external_rows),
            "kept_rows": len(original_all_rows),
            "removed_rows": 0,
    ...
    valid_prob = predict_perox_yes_probabilities(
        rows=valid_rows, perox_model=fold_model
    )
    ...
    report = {
        "dataset": {
            ...
            "all_train_validation_rows": len(all_rows),
            "train_rows_for_threshold_model": len(train_rows),
            ...
            "external_positive_count": int(np.sum(label_vector(external_rows) == 1))
            if external_rows
            else 0,
        },
        ...
        "leakage_checks": {
            ...
            "all_train_validation_vs_external": leakage_report(all_rows, external_rows)
            if external_rows
            else None,
        ...
        "label_quality": {
            ...
            "external": sequence_label_conflict_report(external_rows)
            if external_rows
            else None,
        },
        "metrics": {
    ...
    if str(validation_partition).strip().lower() in [
        "hash_stratified",
        "auto_hash",
        "stratified_hash",
    ]:
        report["threshold_selection"]["tuned_on"] = (
            "hash-stratified validation split (fraction {:.3f})".format(
    ...
    if _str_to_bool(cluster_oof):
        ...
        if source == "external":
            ...
            raise ValueError(
                "Unsupported cluster_oof_source: {}".format(cluster_oof_source)
            )
    ...
    external_prob = None
    if external_rows:
        external_prob = predict_perox_yes_probabilities(
            rows=external_rows, perox_model=final_model
        )
        ...
        if (
            _str_to_bool(homology_check)
            and homology_external is not None
            and homology_external.get("status") == "ok"
        ):
            add_homology_subset_metrics(
                metrics=report["metrics"],
    ...
    if model_out:
        ...
        attached = attach_perox_model_to_base(
            ...
            metadata={
                ...
                "perox_model_exclude_external_from_train": bool(
                    _str_to_bool(exclude_external_from_train)
                ),
...
def _metric_row(name, metrics):
    values = [
        ...
        "{:.4f}".format(float(metrics.get("auprc", 0.0) or 0.0))
        if metrics.get("auprc") is not None
        else "NA",
        "{:.4f}".format(float(metrics.get("auroc", 0.0) or 0.0))
...
def format_report_markdown(report):
    lines = [
        ...
        "- Threshold: `{:.4f}`".format(
            float(report["threshold_selection"]["threshold"])
        ),
    ...
    if isinstance(cluster_oof, dict):
        lines.extend(
            [
                ...
                "- Classification threshold: `{}`".format(
                    cluster_oof.get("classification_threshold", "")
                ),
                "",
        ...
        if "regex_metrics" in cluster_oof:
            lines.append(
                _metric_row("cluster_oof_regex_pts", cluster_oof["regex_metrics"])
            )
        if "constant_zero_metrics" in cluster_oof:
            lines.append(
                _metric_row(
                    "cluster_oof_constant_zero", cluster_oof["constant_zero_metrics"]
                )
            )
        lines.extend(
            [
                ...
                "```json",
                json.dumps(
                    cluster_oof.get("cluster_report", {}), indent=2, sort_keys=True
                ),
                "```",
            ]
        )
...
def build_arg_parser():
    ...
    parser.add_argument(
        "--threshold_objective",
        default="f1",
        choices=["f1", "mcc", "precision", "recall"],
    )
