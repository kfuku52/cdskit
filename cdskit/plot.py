...
from cdskit.atomicio import atomic_output_path, atomic_text_writer
...
def _int_arg(name, value, default):
    ...
    if value > 20_000:
        raise ValueError(f"{name} must be <= 20000 for resource safety. Exiting.\n")
    return value
...
def _normalize_mode(mode):
    ...
    if mode_str not in VALID_PLOT_MODES:
        ...
        raise ValueError(f"Invalid --mode: {mode}. Choose from {valid}. Exiting.\n")
    return mode_str
...
def _normalize_plotformat(plotformat, outfile):
    ...
    if plotformat_str not in VALID_PLOT_FORMATS:
        ...
        raise ValueError(
            f"Invalid --format: {plotformat}. Choose from {valid}. Exiting.\n"
        )
...
def _build_summary_text(
    num_sequences, num_sites, kept_sites, removed_sites, min_clean_fraction
):
    return " | ".join(
        [
...
def _build_state_matrix(records, codontable):
    ...
    for row_idx, record in enumerate(records):
        ...
        for site_idx in range(num_sites):
            ...
            matrix[row_idx, site_idx] = STATE_TO_INT[
                classify_codon(codon=codon, codontable=codontable)
            ]
    return matrix
...
def _build_msa_summary(records, codontable):
    ...
    consensus_aas = list()
    aa_logo_frequencies: list[list[tuple[str, float]]] = []
    ...
    for codon_idx in range(num_codons):
        ...
        if len(valid_codons) > 0:
            ...
            consensus_codon = sorted(
                codon_counts.items(), key=lambda item: (-item[1], item[0])
            )[0][0]
            aa_counts = Counter(
                _translate_codon_for_msa(
                    codon=codon,
            ...
            if len(codons) > 0 and all(
                (len(codon) == 3) and all(ch in "-?." for ch in codon.upper())
                for codon in codons
            ):
                consensus_codon = "---"
            else:
            ...
            aa_logo_frequencies.append(
                [
                    ...
                    for aa, count in sorted(
                        aa_counts.items(), key=lambda item: (item[1], item[0])
                    )
                ]
            )
...
def _draw_msa_rect(
    ax, x, y, width, height, facecolor, edgecolor="#ffffff", linewidth=0.6
):
    ax.add_patch(
        Rectangle(
            (x, y),
...
def _draw_logo_letter(ax, letter, x, y, width, height):
    ...
    ax.add_patch(
        PathPatch(
            text_path,
            transform=transform,
            facecolor=_aa_color_for_char(letter),
            edgecolor="none",
        )
    )
...
def _draw_msa_block(
    ax, records, msa_summary, start, end, wrap, font_size, y_offset, layout
):
    block_len = end - start
    ...
    for row_idx, _record in enumerate(records):
    ...
    for rel_idx in range(0, wrap + 1, 3):
        ...
        ax.plot(
            [x_pos, x_pos],
            [y_offset + layout["aa_tick_top"], block_bottom],
            color=COL_MSA_BORDER,
            linewidth=1.1,
            zorder=0,
        )
...
def _serialize_figure(fig, plotformat):
    if plotformat == "svg":
        ...
        fig.savefig(text_buffer, format="svg", metadata={"Date": None})
        return text_buffer.getvalue()
    binary_buffer = io.BytesIO()
    fig.savefig(binary_buffer, format=plotformat, metadata={"Date": None})
...
def _plot_ambiguous_bar_chart(ax, records, top_n, title):
    if (top_n <= 0) or (len(records) == 0):
        ...
        return
    top_records = sorted(
        records, key=lambda item: (-item["ambiguous_codons"], item["seq_id"])
    )[:top_n]
    if len(top_records) == 0:
        ax.axis("off")
        return
    counts = np.array(
        [record["ambiguous_codons"] for record in top_records], dtype=float
    )
    ...
    for y, count in zip(ypos, counts, strict=False):
        ax.text(
            count + 0.03 * max(1.0, counts.max()),
            y,
            str(int(count)),
            va="center",
            ha="left",
...
def _build_summary_figure(records, site_summaries, kept_sites, ambiguous_by_seq, args):
    ...
    fig.text(
        0.08, 0.90, summary_text, fontsize=10, ha="left", va="top", color="#607d8b"
    )
    ...
    if num_sites == 0:
        ...
        clean_values = np.array(
            [site["clean_fraction"] for site in site_summaries], dtype=float
        )
        stop_positions = x[[site["stop_codons"] > 0 for site in site_summaries]]
        keep_mask = np.array(
            [
                [
                    1 if site_idx in set(kept_sites) else 0
                    for site_idx in range(num_sites)
                ]
            ],
            dtype=int,
        ...
        main_ax.axhline(
            min_clean_fraction, color=COL_THRESHOLD, linewidth=1.2, linestyle="--"
        )
        if len(stop_positions) > 0:
            main_ax.scatter(
                stop_positions,
                np.full(len(stop_positions), 1.04),
                color=COL_STOP,
                marker="v",
                s=32,
                zorder=4,
    ...
    if bar_ax is not None:
        _plot_ambiguous_bar_chart(
            bar_ax, ambiguous_by_seq, top_n, "Top ambiguous sequences"
        )
...
def _build_map_figure(records, summary, args):
    ...
    if show_bar_chart:
        gs = fig.add_gridspec(
            2, 2, width_ratios=[4.0, 1.35], wspace=0.28, **grid_kwargs
        )
        bar_ax = fig.add_subplot(gs[:, 1])
    else:
        gs = fig.add_gridspec(2, 1, **grid_kwargs)
    ...
    if (num_records == 0) or (num_sites == 0):
        ...
        keep_mask = np.array(
            [
                [
                    1 if site_idx in set(summary["kept_sites"]) else 0
                    for site_idx in range(num_sites)
                ]
            ],
            dtype=int,
        ...
        map_ax.imshow(
            state_matrix, aspect="auto", interpolation="nearest", cmap=cmap, norm=norm
        )
    ...
    if bar_ax is not None:
        ...
        _plot_ambiguous_bar_chart(
            bar_ax, top_records, top_n, "Top ambiguous codon counts"
        )
...
def _build_msa_figure(records, args):
    ...
    if wrap % 3 != 0:
        raise ValueError(
            "--wrap must be a multiple of three when --mode=msa. Exiting.\n"
        )
    ...
    block_gap = 0.14
    total_msa_height = (num_blocks * block_layout["total_height"]) + (
        max(0, num_blocks - 1) * block_gap
    )
    unit_px = max(
        38.0, (float(row_height) / max(0.01, block_layout["row_panel_height"])) * 0.56
    )
    ...
    grid_width_fraction = min(
        0.98 - left_margin, (wrap * desired_nt_px) / max(1.0, float(width))
    )
...
def plot_main(args):
    ...
    plotformat = _normalize_plotformat(
        getattr(args, "plotformat", "auto"), outfile=outfile
    )
