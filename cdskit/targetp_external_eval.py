...
def is_exact_targetp_overlap(row, targetp_keys):
    ...
    return (accession != "" and accession in targetp_keys["accessions"]) or (
        sequence != "" and sequence in targetp_keys["sequences"]
    )
...
def build_deeploc_sorting_rows(path, targetp_keys, exclude_exact=True):
    ...
    for row in rows:
        ...
        active = [
            value
            for value in str(row.get("sorting_signal_labels", "")).split(";")
            if value != ""
        ]
        mapped = [
            DEEPLOC_SORTING_TO_TARGETP[value]
            for value in active
            if value in DEEPLOC_SORTING_TO_TARGETP
        ]
        if len(mapped) == 0:
...
def load_fixed_uniprot_holdout_rows(path, strict_targetp_organism_labels=False):
    ...
    if not any(name in fieldnames for name in ["true_class", "localization"]):
        raise ValueError(
            "{} is missing required TSV column: true_class (or localization)".format(
                path
            )
        )
    for row in rows:
...
def filter_rows_by_mmseqs_similarity(
    ...
    target_rows = [
        dict(row, _mmseqs_id="t{}".format(i)) for i, row in enumerate(targetp_rows)
    ]
    with tempfile.TemporaryDirectory(prefix="cdskit_targetp_ext_") as tmpdir:
        ...
        kept = [
            row for row in query_rows if row.get("_mmseqs_id", "") not in hit_queries
        ]
    for row in kept:
...
def compute_single_label_metrics(rows, class_names=LOCALIZATION_CLASSES):
    ...
    accuracy = (
        0.0
        if len(rows) == 0
        else float(sum(1 for t, p in zip(true, pred, strict=False) if t == p))
        / float(len(rows))
    )
    macro_f1 = float(
        np.mean([by_class[class_name]["f1"] for class_name in class_names])
    )
    observed = [
        class_name for class_name in class_names if by_class[class_name]["support"] > 0
    ]
    observed_macro = (
        0.0
        if len(observed) == 0
        else float(np.mean([by_class[class_name]["f1"] for class_name in observed]))
    )
    return {
        "n_rows": len(rows),
    ...
    for row in rows:
        ...
        for class_name in class_names:
            ...
            if column not in row:
                raise ValueError(
                    "Missing prediction probability column: {}.".format(column)
                )
            value = row[column]
            try:
                value = float(value)
...
def _true_indices_from_prediction_rows(rows, class_names=LOCALIZATION_CLASSES):
    ...
    for row in rows:
        ...
        if class_name not in class_to_idx:
            raise ValueError(
                "Unknown true_class in prediction rows: {}".format(class_name)
            )
            ...
            folds_out.append(
                {
                    ...
                    "train_macro_f1": 0.0,
                    "thresholds": {class_name: 1.0 for class_name in class_names},
                }
            )
            continue
...
def render_markdown(result):
    ...
    for key, name in [
        ...
        if calibration:
            ...
            if argmax:
                lines.append(
                    "| Argmax probabilities | {:.3f} | {:.3f} | Raw probability argmax from the model. |".format(
            ...
            if oracle:
                lines.append(
                    "| Oracle thresholds | {:.3f} | {:.3f} | Diagnostic upper bound tuned on all rows; not a fair test estimate. |".format(
            ...
            if cv:
                lines.append(
                    "| {}-fold thresholds | {:.3f} | {:.3f} | Thresholds selected on other folds only. |".format(
...
def run_external_evaluation(
    ...
    if bool(threshold_calibration):
        result["uniprot_holdout"]["threshold_calibration"] = (
...
def build_parser():
    ...
    parser.add_argument(
        "--out_dir", default="data/localize_bench/targetp_external_eval", type=str
    )
    ...
    parser.add_argument(
        "--strict_targetp_organism_labels", default=False, type=parse_bool
    )
...
def main(argv=None):
    ...
    result = run_external_evaluation(
        ...
        strict_targetp_organism_labels=_to_bool_yes_no(
            args.strict_targetp_organism_labels
        ),
    )
