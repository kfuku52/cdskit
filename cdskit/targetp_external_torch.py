...
def _torch_train_kwargs(seq_len, train_kwargs):
    ...
    out.update(
        {
            key: value
            for key, value in dict(train_kwargs or {}).items()
...
def fit_external_augmented_torch_runtime_model(
    ...
    **train_kwargs,
    ...
    seq_len = int(
        train_kwargs.get("seq_len", TARGETP_EXTERNAL_TORCH_DEFAULTS["seq_len"])
    )
    ...
    if str(external_tsv or "").strip() != "":
        ...
        external_report = {
            ...
            "sampled_rows": len(external_rows),
    ...
    calibration_seed = (
        int(external_seed) + 791 if calibration_seed is None else int(calibration_seed)
    )
    ...
    val_arrays = targetp_rows_to_torch_arrays(
        rows=external_calibration_rows, seq_len=seq_len
    )
...
def _parse_grid(text):
    values = [
        float(part.strip()) for part in str(text or "").split(",") if part.strip() != ""
    ]
    if len(values) == 0:
...
def build_parser():
    ...
    parser.add_argument(
        ...
        default=TARGETP_EXTERNAL_TORCH_DEFAULTS["calibration_fraction"],
        type=float,
    )
    parser.add_argument("--calibration_seed", default="", type=str)
    parser.add_argument(
        "--external_seed",
        default=TARGETP_EXTERNAL_TORCH_DEFAULTS["external_seed"],
        type=int,
    )
    parser.add_argument(
        "--seed", default=TARGETP_EXTERNAL_TORCH_DEFAULTS["seed"], type=int
    )
    parser.add_argument("--mmseqs", default=False, type=parse_bool)
    parser.add_argument(
        "--exclusion_mmseqs", default="yes", choices=["auto", "yes", "no"], type=str
    )
    ...
    parser.add_argument(
        "--seq_len", default=TARGETP_EXTERNAL_TORCH_DEFAULTS["seq_len"], type=int
    )
    parser.add_argument(
        "--hidden_rnn", default=TARGETP_EXTERNAL_TORCH_DEFAULTS["hidden_rnn"], type=int
    )
    parser.add_argument(
        "--n_filters", default=TARGETP_EXTERNAL_TORCH_DEFAULTS["n_filters"], type=int
    )
    parser.add_argument(
        "--hidden_fc", default=TARGETP_EXTERNAL_TORCH_DEFAULTS["hidden_fc"], type=int
    )
    parser.add_argument(
        "--n_attention",
        default=TARGETP_EXTERNAL_TORCH_DEFAULTS["n_attention"],
        type=int,
    )
    parser.add_argument(
        "--attention_size",
        default=TARGETP_EXTERNAL_TORCH_DEFAULTS["attention_size"],
        type=int,
    )
    parser.add_argument(
        "--learning_rate",
        default=TARGETP_EXTERNAL_TORCH_DEFAULTS["learning_rate"],
        type=float,
    )
    parser.add_argument(
        "--batch_size", default=TARGETP_EXTERNAL_TORCH_DEFAULTS["batch_size"], type=int
    )
    parser.add_argument(
        "--epochs", default=TARGETP_EXTERNAL_TORCH_DEFAULTS["epochs"], type=int
    )
    parser.add_argument(
        "--patience_epochs",
        default=TARGETP_EXTERNAL_TORCH_DEFAULTS["patience_epochs"],
        type=int,
    )
    parser.add_argument(
        "--max_lr_reductions",
        default=TARGETP_EXTERNAL_TORCH_DEFAULTS["max_lr_reductions"],
        type=int,
    )
    parser.add_argument(
        "--type_class_weight",
        default=TARGETP_EXTERNAL_TORCH_DEFAULTS["type_class_weight"],
        choices=["none", "balanced", "sqrt_balanced", "log_balanced"],
        type=str,
    )
    parser.add_argument(
        "--cleavage_loss_weight",
        default=TARGETP_EXTERNAL_TORCH_DEFAULTS["cleavage_loss_weight"],
        type=float,
    )
    parser.add_argument(
        "--selection_metric",
        default=TARGETP_EXTERNAL_TORCH_DEFAULTS["selection_metric"],
    ...
    parser.add_argument(
        "--initializer",
        default=TARGETP_EXTERNAL_TORCH_DEFAULTS["initializer"],
        choices=["targetp_tf", "pytorch"],
        type=str,
    )
    parser.add_argument(
        "--grad_clip_norm",
        default=TARGETP_EXTERNAL_TORCH_DEFAULTS["grad_clip_norm"],
        type=float,
    )
    parser.add_argument(
        "--rnn_impl",
        default=TARGETP_EXTERNAL_TORCH_DEFAULTS["rnn_impl"],
        choices=["torch_lstm", "targetp_tf_cell"],
        type=str,
    )
    parser.add_argument(
        "--threshold_grid",
...
def main(argv=None):
    ...
    print(
        "target_rows={} external_train_rows={} external_calibration_rows={} best_val_macro_f1={:.6f}".format(
