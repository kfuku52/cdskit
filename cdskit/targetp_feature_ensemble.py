...
def _true_idx_from_rows(rows, class_names):
    ...
    return np.asarray(
        [int(class_to_idx[str(row.get("localization", ""))]) for row in rows],
        dtype=np.int64,
    )
...
def make_targetp_binary_feature_classifiers(
    ...
    if not _is_binary_feature_model_kind(model_kind):
        raise ValueError(
            "Unsupported binary targetp feature model_kind: {}".format(model_kind)
        )
    classifiers = list()
    for class_i, _ in enumerate(class_names):
        ...
        binary_y = (np.asarray(true_idx, dtype=np.int64) == int(class_i)).astype(
            np.int64
        )
...
def predict_binary_feature_classifiers_prob_matrix(
    classifiers, feature_matrix, class_names
):
    if not isinstance(classifiers, list) or len(classifiers) != len(class_names):
        raise ValueError(
            "binary feature classifiers should be a list matching class_names."
        )
    out = np.zeros((feature_matrix.shape[0], len(class_names)), dtype=np.float64)
...
def build_targetp_feature_blend_runtime_model(
    ...
    if feature_model.get("model_type") != "targetp_feature_ensemble_v1":
        raise ValueError(
            "feature_model should have model_type targetp_feature_ensemble_v1."
        )
    if blend_source_model.get("model_type") == "targetp_blend_v1":
        ...
        if blend_base_index < 0 or blend_base_index >= len(base_models):
            raise ValueError(
                "blend_base_index is outside the source targetp_blend base_models."
            )
...
def _prediction_indices_with_thresholds(prob_matrix, thresholds):
    ...
    return np.argmax(
        np.asarray(prob_matrix, dtype=np.float64) / thresholds, axis=1
    ).astype(np.int64)
...
def _optimize_classwise_alpha(prob_a, prob_b, true_idx, class_names, alpha_grid):
    ...
    for trial in alpha_grid:
        pred = np.argmax(
            _blend_classwise(
                prob_a=prob_a,
                prob_b=prob_b,
                alpha_by_class=np.full(
                    (len(class_names),), float(trial), dtype=np.float64
                ),
            ),
            axis=1,
        ).astype(np.int64)
    ...
    if best_metrics is None:
        raise ValueError("alpha_grid should contain at least one value.")
    alpha[:] = best_global
    improved = True
    while improved:
        improved = False
...
def evaluate_foldwise_thresholds(
    prob_matrix, true_idx, fold_ids, class_names, threshold_grid
):
    ...
    for fold_id in sorted(set([str(v) for v in np.asarray(fold_ids).tolist()])):
        valid_mask = np.asarray(
            [str(v) == fold_id for v in np.asarray(fold_ids).tolist()], dtype=bool
        )
        train_mask = ~valid_mask
...
def _render_markdown(out):
    ...
    if "blend_alpha_by_class" in out:
        lines.append(
            "feature/blend alpha by class: {}".format(out["blend_alpha_by_class"])
        )
...
def build_parser():
    ...
    parser.add_argument(
        "--feature_oof_npz",
        default="data/localize_bench/targetp2_oof_feature_ensemble.npz",
        type=str,
    )
    parser.add_argument("--organism_gate", default=True, type=parse_bool)
    parser.add_argument(
        "--model_kind",
        default=TARGETP_FEATURE_ENSEMBLE_DEFAULTS["model_kind"],
        choices=[
            "extra_trees",
            "random_forest",
            "binary_extra_trees",
    ...
    parser.add_argument(
        "--n_estimators",
        default=TARGETP_FEATURE_ENSEMBLE_DEFAULTS["n_estimators"],
        type=int,
    )
    parser.add_argument(
        "--random_state",
        default=TARGETP_FEATURE_ENSEMBLE_DEFAULTS["random_state"],
        type=int,
    )
    parser.add_argument(
        "--class_weight",
        default=TARGETP_FEATURE_ENSEMBLE_DEFAULTS["class_weight"],
        type=str,
    )
    parser.add_argument(
        "--max_features",
        default=TARGETP_FEATURE_ENSEMBLE_DEFAULTS["max_features"],
        type=str,
    )
    parser.add_argument(
        "--min_samples_leaf",
        default=TARGETP_FEATURE_ENSEMBLE_DEFAULTS["min_samples_leaf"],
        type=int,
    )
    parser.add_argument(
        "--threads",
    ...
    parser.add_argument(
        "--out_json",
        default="data/localize_bench/targetp2_feature_ensemble_eval.json",
        type=str,
    )
    parser.add_argument(
        "--out_md",
        default="data/localize_bench/targetp2_feature_ensemble_eval.md",
        type=str,
    )
    return parser
...
def main(argv=None):
    ...
    if reuse_cache and os.path.exists(args.feature_oof_npz):
        ...
        if names != class_names:
            raise ValueError(
                "Class names in feature_oof_npz do not match LOCALIZATION_CLASSES."
            )
    ...
    if str(args.blend_oof_npz).strip() != "":
        ...
        if blend_names != class_names:
            raise ValueError(
                "Class names in blend_oof_npz do not match LOCALIZATION_CLASSES."
            )
        if np.any(blend_true != true_idx):
            raise ValueError(
                "True labels in blend_oof_npz do not match feature OOF labels."
            )
        if organism_gate:
            plant_mask = _read_organism_group_mask(training_tsv=args.training_tsv)
        ...
        alpha_grid = sorted(
            set(
                [round(i * step, 10) for i in range(int(np.floor(1.0 / step)) + 1)]
                + [1.0]
            )
        )
        ...
        results["feature_{}_foldwise".format(args.blend_label)] = (
    ...
    if str(args.model_out).strip() != "" or str(args.blend_model_out).strip() != "":
        ...
        export_feature_model["metadata"]["benchmark_feature_threshold_macro_f1"] = (
            float(feature_threshold_metrics["macro_f1"])
        )
        export_feature_model["metadata"][
            "benchmark_feature_foldwise_threshold_macro_f1"
        ] = float(feature_foldwise["metrics"]["macro_f1"])
    ...
    if str(args.blend_model_out).strip() != "":
        if blend_prob is None:
            raise ValueError(
                "--blend_model_out requires --blend_oof_npz for calibration."
            )
        if str(args.blend_model).strip() == "":
            raise ValueError(
                "--blend_model_out requires --blend_model as the runtime source."
            )
        ...
        blend_runtime = build_targetp_feature_blend_runtime_model(
            ...
            metadata={
                ...
                "benchmark_feature_blend_threshold_macro_f1": float(
                    results["feature_{}_threshold".format(args.blend_label)]["metrics"][
                        "macro_f1"
                    ]
                ),
                "benchmark_feature_blend_foldwise_macro_f1": float(
                    results["feature_{}_foldwise".format(args.blend_label)]["metrics"][
                        "macro_f1"
                    ]
                ),
            },
