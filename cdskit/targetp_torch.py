...
def targetp_blosum62_probability_table():
    ...
    uniform = np.full(
        (len(TARGETP_BLOSUM62_ORDER),),
        1.0 / len(TARGETP_BLOSUM62_ORDER),
        dtype=np.float32,
    )
    table["B"] = uniform
    table["J"] = uniform
...
def _to_targetp_blosum_sequence(aa_seq):
    ...
    if "*" in seq:
        raise ValueError(
            "Internal stop codon detected in translated peptide sequence. Exiting."
        )
...
def encode_targetp_blosum_sequences(aa_sequences, seq_len=200, blosum_table=None):
    ...
    table = (
        targetp_blosum62_probability_table() if blosum_table is None else blosum_table
    )
    pad_vec = np.asarray(table["<PAD>"], dtype=np.float32)
    x = np.zeros(
        (len(aa_sequences), seq_len, len(TARGETP_BLOSUM62_ORDER)), dtype=np.float32
    )
    ...
    for row_i, aa_seq in enumerate(aa_sequences):
        ...
        if use_len > 0:
            for pos_i in range(use_len):
                x[row_i, pos_i, :] = np.asarray(
                    table.get(seq[pos_i], table["X"]), dtype=np.float32
                )
        if use_len < seq_len:
...
def targetp_rows_to_torch_arrays(
    ...
    rows = list(rows)
    class_to_idx = {
        class_name: class_i for class_i, class_name in enumerate(LOCALIZATION_CLASSES)
    }
    ...
    for row_i, row in enumerate(rows):
        ...
        if class_name not in class_to_idx:
            raise ValueError(
                "Unknown TargetP localization label: {}".format(class_name)
            )
...
def _targetp_metrics_from_prediction_indices(true_idx, pred_idx, class_names):
    ...
    return {
        ...
        "macro_f1": float(np.mean(np.asarray(f1_values, dtype=np.float64)))
        if len(f1_values) > 0
        else 0.0,
        "by_class": by_class,
    }
    ...
    class TargetPTFLSTMCell(nn.Module):
        def __init__(self, input_size, hidden_size, forget_bias=1.0):
            ...
            self.kernel = nn.Parameter(
                torch.empty(self.input_size + self.hidden_size, 4 * self.hidden_size)
            )
        ...
        def forward(self, x, state):
            ...
            next_c = c * torch.sigmoid(f_gate + self.forget_bias) + torch.sigmoid(
                i_gate
            ) * torch.tanh(j_gate)
    ...
    class TargetP2TorchNet(nn.Module):
        def __init__(self):
            ...
            self.conv_dropout = nn.Dropout(
                p=max(0.0, min(1.0, 1.0 - float(encoder_keep_prob)))
            )
            self.organism_fc = nn.Linear(int(n_org), int(hidden_rnn))
            if self.rnn_impl == "targetp_tf_cell":
                self.fw_cell = TargetPTFLSTMCell(
                    input_size=int(n_filters), hidden_size=int(hidden_rnn)
                )
                self.bw_cell = TargetPTFLSTMCell(
                    input_size=int(n_filters), hidden_size=int(hidden_rnn)
                )
            else:
        ...
        def _run_tf_style_lstm_cell(self, conv, lengths, org_state, rnn_keep_prob):
            ...
            zero_out = torch.zeros(
                (batch_size, hidden_size), dtype=conv.dtype, device=conv.device
            )
        ...
        def forward(self, x, lengths, organism, rnn_keep_prob=1.0):
            ...
            org_hot = F.one_hot(organism.long().clamp(min=0, max=1), num_classes=2).to(
                dtype=x.dtype
            )
            ...
            mask = torch.arange(self.seq_len, device=x.device).unsqueeze(
                0
            ) < lengths.unsqueeze(1)
...
def initialize_targetp2_torch_module(torch, nn, module, initializer="targetp_tf"):
    ...
    if initializer != "targetp_tf":
        raise ValueError(
            "Unsupported TargetP torch initializer: {}".format(initializer)
        )
...
def _targetp_type_class_weight_vector(y_type_train, mode, num_class):
    ...
    if mode not in ["balanced", "sqrt_balanced", "log_balanced"]:
        raise ValueError(
            "type_class_weight should be none, balanced, sqrt_balanced, or log_balanced."
        )
    counts = np.bincount(
...
def _predict_encoded_with_module(torch, module, device, x, lengths, org, batch_size):
    ...
    with torch.no_grad():
        for batch_idx in _iter_minibatches(
            x.shape[0], batch_size, np.random.default_rng(1), shuffle=False
        ):
...
def _evaluate_encoded(
    ...
    prob = (
        ...
        if len(probs) > 0
        else np.zeros((0, len(LOCALIZATION_CLASSES)))
    )
    pred = (
        np.argmax(prob, axis=1).astype(np.int64)
        if prob.shape[0] > 0
    ...
    return {
        ...
        "accuracy": float(np.mean(pred == np.asarray(y_type, dtype=np.int64)))
        if prob.shape[0] > 0
        else 0.0,
        "prob_matrix": prob,
    }
...
def _targetp_torch_payload(
    ...
    payload = {
        ...
        else _state_dict_to_cpu(module),
        ...
        "current_lr": float(
            current_lr if current_lr is not None else config.get("learning_rate", 0.0)
        ),
...
def fit_targetp2_torch_model(
    ...
    **kwargs,
    ...
    config = merge_targetp_training_config(
        defaults=TARGETP_TORCH_DEFAULTS,
        overrides=kwargs,
        resume_payload=resume_payload,
        resume_state_impl=resume_state_impl,
    )
    ...
    if weights is not None:
        type_weight = torch.as_tensor(
            weights, dtype=torch.float32, device=resolved_device
        )

    options = parse_targetp_training_options(
        config,
        default_threshold_grid=TARGETP_CLASS_THRESHOLD_GRID,
    )
    batch_size = options.batch_size
    epochs = options.epochs
    balanced_batch = options.balanced_batch
    selection_metric = options.selection_metric
    selection_threshold_grid = options.selection_threshold_grid
    resume_state = options.resume_state
    resume_learning_rate = options.resume_learning_rate
    resume_reset_optimizer = options.resume_reset_optimizer
    resume_reset_scheduler = options.resume_reset_scheduler
    resume_reset_best_metrics = options.resume_reset_best_metrics
    ...
    if isinstance(resume_payload, dict):
        if resume_state == "best":
            state = resume_payload.get(
                "state_dict", resume_payload.get("latest_state_dict")
            )
        else:
            state = resume_payload.get(
                "latest_state_dict", resume_payload.get("state_dict")
            )
        ...
        if (
            ...
            and resume_payload.get("torch_cuda_rng_state_all") is not None
        ):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if balanced_batch:
            ...
            batch_iter = _iter_minibatches(
                x_train.shape[0], batch_size, rng, shuffle=True
            )
        for batch_idx in batch_iter:
            xb = torch.as_tensor(
                x_train[batch_idx], dtype=torch.float32, device=resolved_device
            )
            yb = torch.as_tensor(
                y_type_train[batch_idx], dtype=torch.long, device=resolved_device
            )
            cb = torch.as_tensor(
                y_cs_train[batch_idx], dtype=torch.long, device=resolved_device
            )
            lb = torch.as_tensor(
                len_train[batch_idx], dtype=torch.long, device=resolved_device
            )
            ob = torch.as_tensor(
                org_train[batch_idx], dtype=torch.long, device=resolved_device
            )
        ...
        if selection_metric == "val_threshold_macro_f1":
            ...
            row["val_threshold_accuracy"] = float(
                val_threshold_metrics["overall_accuracy"]
            )
            row["val_thresholds"] = {
        ...
        is_better = targetp_metric_is_better(
            row=row,
            best_metrics=best_metrics,
        ...
        if is_better:
            ...
            best_state = copy.deepcopy(_state_dict_to_cpu(module))
        if should_reduce_learning_rate(
            epoch=int(epoch + 1),
            lr_reference_epoch=int(lr_reference_epoch),
...
def predict_targetp2_torch_encoded(
    x, lengths, org, localization_model, device="cpu", batch_size=512
):
...
def predict_targetp2_torch_batch(
    aa_sequences, organism_groups, localization_model, device="cpu", batch_size=512
):
    config = dict(TARGETP_TORCH_DEFAULTS)
    ...
    org = np.asarray(
        [organism_group_to_targetp_org(group) for group in organism_groups],
        dtype=np.int64,
    )
...
def export_targetp2_torch_localize_model(
    model_payload, training_tsv="", class_thresholds=None
):
    thresholds = class_thresholds
    if thresholds is None:
        best_metrics = (
            model_payload.get("best_metrics", {})
...
def _parse_fold_subset(value, available):
    ...
    for part in text.split(","):
        ...
        if fold_id not in available:
            raise ValueError(
                "Fold {} is not available. Available folds: {}".format(
                    fold_id, available
                )
            )
        out.append(fold_id)
...
def parse_targetp_fold_pairs(value, available=None):
    ...
    for part in text.split(","):
        ...
        if outer == val:
            raise ValueError(
                "Outer and validation folds should differ: {}".format(part)
            )
        if available_set is not None and (
            outer not in available_set or val not in available_set
        ):
...
def run_targetp2_torch_nested_oof(
    ...
    **train_kwargs,
    ...
    prob_sum = np.zeros(
        (data["x"].shape[0], len(LOCALIZATION_CLASSES)), dtype=np.float64
    )
    ...
    for outer_fold in selected_outer:
        ...
        selected_val = [
            fold for fold in selected_val_all if int(fold) in inner_candidates
        ]
        for val_fold in selected_val:
            if int(max_models) > 0 and num_trained_or_loaded >= int(max_models):
                break
            train_mask = np.isin(
                folds, [fold for fold in inner_candidates if int(fold) != int(val_fold)]
            )
            ...
            if bool(reuse_cache) and os.path.exists(checkpoint):
                ...
                requested_epochs = int(
                    train_kwargs.get("epochs", TARGETP_TORCH_DEFAULTS["epochs"])
                )
                cached_epochs = len(payload.get("history", []))
                if (
                    bool(payload.get("training_complete", False))
                    and cached_epochs >= requested_epochs
                ):
                    used_cache = True
                else:
            ...
            prob = predict_targetp2_torch_encoded(
                ...
                device=device,
                batch_size=int(
                    train_kwargs.get("batch_size", TARGETP_TORCH_DEFAULTS["batch_size"])
                ),
            )
            ...
            if bool(val_threshold_eval):
                val_prob = predict_targetp2_torch_encoded(
                    ...
                    batch_size=int(
                        train_kwargs.get(
                            "batch_size", TARGETP_TORCH_DEFAULTS["batch_size"]
                        )
                    ),
                )
    ...
    prob_matrix[covered, :] = prob_sum[covered, :] / prob_count[covered].reshape(
        (-1, 1)
    )
    if np.any(~covered):
    ...
    metrics = {
        ...
        "macro_f1": _macro_f1_from_indices(
            data["y_type"][covered], pred, len(LOCALIZATION_CLASSES)
        )
        if np.any(covered)
        else 0.0,
        "overall_accuracy": float(np.mean(pred == data["y_type"][covered]))
        if np.any(covered)
        else 0.0,
    }
    result = {
    ...
    if bool(val_threshold_eval):
        ...
        threshold_score_matrix[threshold_covered, :] = threshold_score_sum[
            threshold_covered, :
        ...
        threshold_pred = np.argmax(
            threshold_score_matrix[threshold_covered, :], axis=1
        ).astype(np.int64)
...
def run_targetp2_torch_paired_oof(
    ...
    **train_kwargs,
    ...
    for outer_fold, val_fold in pairs:
        ...
        prob_sum[covered, :] += np.asarray(result["prob_matrix"], dtype=np.float64)[
            covered, :
        ]
        prob_count[covered] += np.asarray(result["prob_count"], dtype=np.int64)[covered]
        if bool(val_threshold_eval):
            threshold_covered = (
                np.asarray(result["val_threshold_count"], dtype=np.int64) > 0
            )
    ...
    pred = np.argmax(prob_matrix[covered, :], axis=1).astype(np.int64)
    metrics = (
        _targetp_metrics_from_prediction_indices(
...
def write_targetp2_torch_oof_npz(path, result):
    ...
    if "val_threshold_score_matrix" in result:
        ...
        arrays["val_threshold_count"] = np.asarray(
            result["val_threshold_count"], dtype=np.int64
        )
...
def compose_targetp_torch_oof_replacements(
    base_oof_npz, replacement_oof_npzs, source="val_threshold"
):
    ...
    for key in required:
        if key not in base:
            raise ValueError(
                "Base OOF cache is missing {}: {}".format(key, base_oof_npz)
            )
    ...
    if (
        prob_matrix.shape[0] != true_idx.shape[0]
        or prob_matrix.shape[0] != fold_ids.shape[0]
    ):
        raise ValueError(
            "Base OOF cache has inconsistent row counts: {}".format(base_oof_npz)
        )
    ...
    for replacement_path in replacement_oof_npzs:
        ...
        for key in required:
            if key not in replacement:
                raise ValueError(
                    "Replacement OOF cache is missing {}: {}".format(
                        key, replacement_path
                    )
                )
        if not np.array_equal(
            true_idx, np.asarray(replacement["true_idx"], dtype=np.int64)
        ):
            raise ValueError(
                "Replacement true labels do not match base OOF: {}".format(
                    replacement_path
                )
            )
        if not np.array_equal(
            fold_ids, np.asarray(replacement["fold_ids"], dtype=np.int64)
        ):
            raise ValueError(
                "Replacement fold ids do not match base OOF: {}".format(
                    replacement_path
                )
            )
        if not np.array_equal(class_names, np.asarray(replacement["class_names"])):
            raise ValueError(
                "Replacement class names do not match base OOF: {}".format(
                    replacement_path
                )
            )
        if source == "val_threshold":
            if (
                "val_threshold_score_matrix" not in replacement
                or "val_threshold_count" not in replacement
            ):
                raise ValueError(
                    "Replacement OOF cache has no val-threshold scores: {}".format(
                        replacement_path
                    )
                )
            replacement_prob = _normalize_targetp_score_rows(
                replacement["val_threshold_score_matrix"]
            )
        ...
        if replacement_prob.shape != prob_matrix.shape:
            raise ValueError(
                "Replacement probability matrix shape mismatch: {}".format(
                    replacement_path
                )
            )
        if covered.shape[0] != prob_matrix.shape[0]:
            raise ValueError(
                "Replacement covered row count mismatch: {}".format(replacement_path)
            )
        if np.any(replaced & covered):
            overlap_folds = sorted(
                set(fold_ids[replaced & covered].astype(int).tolist())
            )
            raise ValueError(
                "Replacement OOF caches overlap on folds {}: {}".format(
                    overlap_folds, replacement_path
                )
            )
