...
class _FixedBinaryClassifier:
    ...
    def predict_proba(self, features):
        ...
        row = np.asarray(
            [1.0 - self.yes_probability, self.yes_probability], dtype=np.float64
        )
...
def test_deeploc21_perox_benchmark_fixture(temp_dir):
    ...
    assert (
        report["label_quality"]["all_train_validation"]["conflicting_sequence_count"]
        == 0
    )
    assert report["cluster_oof"]["status"] == "ok"
    assert report["cluster_oof"]["metrics"]["rows"] == 4
    assert (
        report["leakage_checks"]["homology_threshold_train_vs_validation"]["status"]
        == "skipped"
    )
...
def test_deeploc21_perox_benchmark_writes_attached_model(temp_dir):
    ...
    assert loaded["perox_model"]["prediction_scope"].startswith(
        "peroxisome sequence-label"
    )
