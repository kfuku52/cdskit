...
def test_build_external_augmented_rows_can_filter_mmseqs_similarity(
...
def test_build_external_augmented_rows_filters_holdout_similarity(
    temp_dir, monkeypatch
):
    calls = list()
    ...
    def fake_filter_rows_by_mmseqs_similarity(
        ...
        kept = [
            row for row in rows if not (remove_u2 and row.get("accession", "") == "U2")
        ]
        return kept, {
...
def test_fit_external_augmented_feature_runtime_model_records_external_training(
    temp_dir,
):
    targetp = temp_dir / "targetp.tsv"
    uniprot = temp_dir / "uniprot.tsv"
    ...
    assert (
        model["localization_model"]["classifier_profile"]["external_augmented"] is True
    )
