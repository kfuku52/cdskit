...
def test_targetp_blosum_encoder_matches_official_probability_rows():
    ...
    np.testing.assert_allclose(
        x[0, 2, :], np.full((20,), 0.05), rtol=1.0e-6, atol=1.0e-7
    )
    np.testing.assert_allclose(
        x[0, 3, :], np.full((20,), 0.05), rtol=1.0e-6, atol=1.0e-7
    )
...
def test_targetp_tf_initializer_sets_lstm_forget_bias():
    ...
    for name, param in module.encoder.named_parameters():
        if "bias" in name:
            ...
            np.testing.assert_allclose(
                param.detach().cpu().numpy()[hidden : 2 * hidden], expected
            )
...
def test_targetp_tf_cell_impl_has_single_kernel_bias_and_outputs():
    ...
    for cell in [module.fw_cell, module.bw_cell]:
        ...
        np.testing.assert_allclose(
            cell.bias.detach().cpu().numpy(), np.zeros((4 * 4,), dtype=np.float32)
        )
...
def test_targetp_torch_detects_rnn_impl_from_state_dict():
    ...
    assert (
        _payload_rnn_impl_from_state({"state_dict": legacy.state_dict()})
        == "torch_lstm"
    )
    assert (
        _payload_rnn_impl_from_state({"state_dict": tf_cell.state_dict()})
        == "targetp_tf_cell"
    )
...
def test_targetp_type_class_weight_modes_are_ordered_and_normalized():
    ...
    sqrt_balanced = _targetp_type_class_weight_vector(
        y, mode="sqrt_balanced", num_class=5
    )
    log_balanced = _targetp_type_class_weight_vector(
        y, mode="log_balanced", num_class=5
    )
...
def test_targetp_loss_can_disable_cleavage_auxiliary_term():
    ...
    assert float(with_cleavage.detach().cpu().item()) > float(
        type_only.detach().cpu().item()
    )
